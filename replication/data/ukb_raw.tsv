...
<<<<<<< HEAD
        dodge=True, palette="Set2", size=5, height=5, aspect=1, hue_order=hue_order)
=======
        dodge=True, palette="Set2", size=10, height=5, aspect=1, hue_order=hue_order)
>>>>>>> origin/master
    plt.savefig(outfile)
    plt.close()
    return
...
<<<<<<< HEAD
    pheno_list_curr = pheno_col_list.copy()
    for i, gender in enumerate(["female", "male"]):
        ...
        if pheno_type in comp_cols.keys():
            dtypes_pheno.update({col: float for col in comp_cols[pheno_type]})
...
<<<<<<< HEAD
corr_all["Absolute r"] = abs(corr_all["r"])
corr_all.to_csv(Path(args.out_dir, "ukb_dep-pheno_corr.csv"))

# Significant correlations (correcting for multiple comparisons)
